 ...
 *
 * The grain texture is a global SVG filter declared once in <ExploreNoise />
 * and referenced here via CSS background — see app/explore/page.tsx.
 */
export function Stage({ title, caption, children, className }: StageProps) {
  return (
    <section
      className={cn(
        'relative flex flex-col gap-5 border border-[var(--color-border-subtle)] bg-[var(--color-paper)] p-6 text-[var(--color-ink)]',
    ...
    >
      <header className="relative flex items-baseline justify-between gap-4">
        <h3 className="font-serif text-lg leading-tight tracking-tight text-[var(--color-ink)]">
          {title}
        </h3>
        {caption && (

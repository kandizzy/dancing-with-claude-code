...
const F2_SYSTEM = `You are Claude, the co-pilot in a browser CV playground. The user just invoked a slash command. Honor whatever the slash command body asks for. Keep replies short (1–3 short paragraphs).`
...
export function Level2Workspace({ level }: Props) {
  ...
  const send = useCallback(async () => {
    ...
    try {
      const result = await ask({ systemPrompt: F2_SYSTEM, userPrompt: input })
      setMessages((m) => [
        ...m,
  ...
  }, [input, streaming, pickedCommand, level, awardShape, isCompleted])
